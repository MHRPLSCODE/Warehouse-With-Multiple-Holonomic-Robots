...
class HolonomicController(Node):
    def __init__(self):
        super().__init__('holonomic0_controller')
        ...
        # ---------------- IDs ----------------     # UNIQUE TO BOT
        self.BOT_ID = 0
        self.IGNORE_BOT_ID = 4
        self.assigned_crate_id = None
        
        self.stack_flag = True
        
        self.drop_subphase = 0.0
        self.drop_subphase_time = None
        self.DROP_CMD_DELAY = 3.0  # seconds
        ...
        # ---------------- PID ----------------     # UNIQUE TO BOT
        self.pid_x = PID(6.0, 0.0, 0.8, 30.0)
        self.pid_y = PID(6.0, 0.0, 0.8, 30.0)


        # ---------------- arm angles ----------------     # UNIQUE TO BOT
        self.default_elbow = 90.0
        self.pickup_elbow = 0.0
        self.default_base = 35.0
        self.drop_base =  25.0
        self.rotate_base = 25.0
        self.stack_lift_base = 80.0
        ...
        self.BRAKE_RADIUS = 40.0   # start slowing down here
        self.ignore_flag = False

        self.pickup_start_time = None
        self.PICKUP_DELAY = 30.0  # seconds (tune this)
        ...
        self.state_ir = True
        self.prev_ir = True
        self.create_subscription(Bool, '/bot0/ir', self.ir_cb, 10)      #UNIQUE TO BOT
    ...
    def control_loop(self):
        ...
        if self.state == "PATH_TO_CRATE":
            ...
            if (self.wp_idx_crate == len(self.path_crate) - 2):
                self.final_tx, self.final_ty = self.path_crate[self.wp_idx_crate]
                print("Final crate cell recorded: ", self.final_tx," ", self.final_ty)
            tx, ty = self.path_crate[self.wp_idx_crate]
            final = (self.wp_idx_crate == len(self.path_crate) - 1)

            if (self.wp_idx_crate == len(self.path_crate) - 1):
                if self.drive_to(self.final_tx, self.final_ty, final):
                    print("Inside move_to_crate, index: ", self.wp_idx_crate)
                    self.wp_idx_crate += 1
                    print("CRATE_POSE AS USED: ", self.final_tx, " ", self.final_ty)
                    print("Inside move_to_crate, index: ", self.wp_idx_crate)
            else:
                if self.drive_to(tx, ty, final):
                    self.wp_idx_crate += 1
                    print("INDEX:", self.wp_idx_crate, "CRATE_POSE USED: ", tx, " ", ty)
            ...
            self.drop_subphase_time = self.get_clock().now().nanoseconds * 1e-9
            if self.pickup_start_time is None:
                # first entry into PICKUP
                self.pickup_start_time = self.get_clock().now().nanoseconds / 1e9
                #lol
                now=self.get_clock().now().nanoseconds /1e9
                self.pickup_start_time=now
                self.we=0
                self.drop_subphase_time=None
                if(self.stack_flag):
                    if self.drop_subphase==0:
                        self.publish_cmd(  0, 0, 0,
                        self.stack_drop_base,
                        self.stack_drop_elbow
                        )
                        self.drop_subphase=1
                        self.drop_subphase_time=now
                    if self.drop_subphase==1:
                        if now-self.drop_subphase_time<self.DROP_CMD_DELAY:
                            return
                        self.drop_subphase=2
                    if self.drop_subphase==2:
                        self.publish_cmd(0,0,0,70,40)
                        self.drop_subphase=3
            ...
            if self.crates_delivered < self.MAX_CRATES:
                self.notify_planner()
                print("Notified the planner 1")
                self.notify_planner()
                print("Notified the planner 2")
                self.notify_planner()
                print("Notified the planner 3")
                self.state = "PATH_TO_CRATE"
                ...
                # print("Notified the planner")
                self.stack_flag = True
                self.state = "HALFWAY_HOME"
                print("HALFWAY_HOME_STARTED")

        
        elif self.state == "HALFWAY_HOME":
            self.halfway_home_path = [(677.0, 1220.0), (724.0, 777.0,), (1180.0, 748.0)]
            self.halfway_home_idx = 0
            self.state = "MOVE_TO_HALFWAY"
            self.get_logger().info("HALFWAY_HOME → PATH_TO_HOME")
            return
        
        elif self.state == "MOVE_TO_HALFWAY":
            if self.halfway_home_idx >= len(self.halfway_home_path):
                self.stop_robot()
                self.state = "PATH_TO_HOME"
                #self.tx, self.ty = self.crate_pose.x, self.crate_pose.y
                self.notify_planner()
                print("Notified the planner HOME BOT 0")
                self.get_logger().info("MOVE_TO_HALFWAY STATE DONE")
                self.get_logger().info("PATH_TO_HOME STATE START")
                return

            tx, ty = self.halfway_home_path[self.halfway_home_idx]
            final = (self.halfway_home_idx == len(self.halfway_home_path) - 1)

            if self.drive_to(tx, ty, final):
                self.halfway_home_idx += 1
        ...
        # -------- PLAN RETURN HOME (NEW) --------
            ...
            if abs(err) < math.radians(4.0):  # ~2 deg tolerance
